...
def compute_min_ess(arviz_draws):
    ...
    posterior = arviz_draws.posterior.drop_vars('player_skills', errors='ignore')
    ess = az.ess(posterior)

    return float(ess.to_array().min().values)
```
...
to_plot = rel_results[rel_results['n_matches'] == rel_results['n_matches'].max()].sort_values('ESS / second')
...
stan_res = az.from_netcdf(join(run_dir, 'cmdstanpy/samples_1970.netcdf'))
```
...
```{python}
pymc3_res = az.from_netcdf(join(run_dir, 'pymc/samples_1970.netcdf'))
```
...
```{python}
# Load data for 1970 to get player names
cur_data = create_arrays(1970)

jax_res = az.from_netcdf(join(run_dir, 'pymc_numpyro_gpu_vectorized/samples_1970.netcdf'))
```
